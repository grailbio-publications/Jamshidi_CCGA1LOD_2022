...
plot_mult_roc_curve <- function(ls_predictions,
  ...
  if (identical(vec_colors, "")) {
    vec_colors <- gg_color_hue(length(ls_roc))
  }
  if (identical(vec_fills, "")) {
    vec_fills <- rep("white", length(ls_roc))
  }
  if (identical(vec_linetypes, "")) {
...
my_blue <- rgb(2, 115, 179, maxColorValue = 255)
my_green <- rgb(2, 158, 115, maxColorValue = 255)
my_purple <- rgb(141, 144, 212, maxColorValue = 255)
my_darkred <- rgb(170, 51, 120, maxColorValue = 255)
my_orange <- rgb(241, 101, 34, maxColorValue = 255)
my_magenta <- rgb(235, 0, 139, maxColorValue = 255)
my_cyan <- rgb(0, 173, 238, maxColorValue = 255)
my_gray <- rgb(146, 148, 151, maxColorValue = 255)
my_yellow <- rgb(250, 175, 64, maxColorValue = 255)
my_black <- rgb(64, 64, 65, maxColorValue = 255)

df_roc_format <- tibble::tribble(
  ~classifier_name,     ~colors,      ~fills,     ~linetypes,
  "WG methylation",     my_orange,    my_orange,    "solid",
  "SNV",                my_blue,      my_blue,      "solid",
  "SNV-WBC",            my_magenta,   my_magenta,   "solid",
  "SCNA",               my_green,     my_green,     "solid",
  "SCNA-WBC",           my_cyan,      my_cyan,      "solid",
  "Fragment endpoints", my_gray,      my_gray,      "solid",
  "Fragment lengths",   my_darkred,   my_darkred,   "solid",
  "Allelic imbalance",  my_black,     my_black,     "solid",
  "Pan-feature",        my_purple,    my_purple,    "solid",
  "Clinical data",      my_yellow,    my_yellow,    "solid"
)
...
make_ctaf_vs_stage_plot <- function(score_and_ctaf,
  ...
  df_ctaf_counts <- df_scores_cnc_w_ctaf %>%
    ...
    tidyr::complete(cstage, called_positive, fill = list(n = 0L)) %>%
...
plt_var_valid <- ggplot2::ggplot() +
  ...
  ggplot2::scale_color_manual(
    ...
    breaks = c(TRUE, FALSE),
    labels = c("Cancer signal detected", "Cancer signal not detected")) +
